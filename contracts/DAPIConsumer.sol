...
contract DAPIConsumer is ChainlinkClient, ConfirmedOwner {
    using Chainlink for Chainlink.Request;

    string public bestBlockHash;
    uint256 private fee;

    event RequestBestBlockHash(bytes32 indexed requestId, string bestBlockHash);

    constructor() ConfirmedOwner(msg.sender) {
        setChainlinkToken(0x01BE23585060835E02B77ef475b0Cc51aA1e0709);
        setChainlinkOracle(0x7B54593DfE8f6d23D4fe6a7d542889DC61498BE4);
        fee = (1 * LINK_DIVISIBILITY) / 10; // 0,1 * 10**18 (Varies by network and job)
    }

    /**
     * Create a Chainlink request to retrieve Dash last block hash
     */
    function requestBestBlockHash(string memory _jobId)
        public
        returns (bytes32 requestId)
    {
        ...
        req.add("post", "http://seed-1.testnet.networks.dash.org:3000");

        // Set the body of the request
        req.add(
            "body",
            '{"id":0,"jsonrpc":"2.0","method":"getBestBlockHash","params":{}}'
        );
     ...
     */
    function fulfill(bytes32 _requestId, string memory _bestBlockHash)
        public
        recordChainlinkFulfillment(_requestId)
    {
        emit RequestBestBlockHash(_requestId, _bestBlockHash);
